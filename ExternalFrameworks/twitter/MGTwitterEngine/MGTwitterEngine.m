...
@synthesize loginController,aHUD,twitLoginView;
...
- (void) showHUD
{
	[aHUD loadingViewInView:twitLoginView text:@"Logging in..."];
}
...
- (void)connection:(MGTwitterHTTPURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    else if (statusCode == 304 || [connection responseType] == MGTwitterGeneric)
   {
	...
	[self killHUD];
	[twitLoginView sendStatusCode:_statusCode];
...
- (NSString *)checkUserCredentials
{
    ...
    NSString *temp =  [self _sendRequestWithMethod:nil path:path queryParameters:nil body:nil 
