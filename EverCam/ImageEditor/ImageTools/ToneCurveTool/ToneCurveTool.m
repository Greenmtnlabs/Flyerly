...
#import "ToneCurveTool.h"
#import "SplineInterpolator.h"
#import "Configs.h"


@protocol ToneCurveGridDelegate;

@interface ToneCurveView : UIView

@property (nonatomic, weak) id<ToneCurveGridDelegate> delegate;
...
@protocol ToneCurveGridDelegate <NSObject>
@required
- (void)toneCurveDidChange:(ToneCurveView*)view;
@end


@interface ToneCurveTool()
<ToneCurveGridDelegate>
@end

@implementation ToneCurveTool
{
    UIImage *_originalImage;
    UIImage *_thumnailImage;
    
    UIView *_menuContainer;
    ToneCurveView *_tonecurveView;
...
+ (NSString*)defaultTitle {
    return NSLocalizedString(@"ToneCurveTool", @"");
}
...
- (void)setup {
    ...
    _tonecurveView = [[ToneCurveView alloc] initWithSuperview:_menuContainer frame:CGRectMake(10, 20, 240, 240)];
    ...
    [btn setImage:[UIImage imageNamed:@"tc_arrowButt"] forState:UIControlStateNormal];
    ...
    [btn setImage:[UIImage imageNamed:@"tc_resetButt"] forState:UIControlStateNormal];
    [_menuContainer addSubview:btn];
...
- (void)pushedResetBtn:(UIButton*)sender
{
    ...
    rotation.duration = kImageToolAnimationDuration;
...
- (void)drawRect:(CGRect)rect
{
    ...
    SplineInterpolator *spline = [[SplineInterpolator alloc] initWithPoints:points];
