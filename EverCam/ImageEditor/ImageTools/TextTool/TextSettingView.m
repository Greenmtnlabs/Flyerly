...
@implementation TextSettingView
{
    ...
    FontPickerView *_fontPickerView;
    
    UIView *_colorPanel;
    CircleView *_fillCircle;
    CircleView *_pathCircle;
...
- (void)setColorPanel {
    _colorPickerView = [[ColorPickerView alloc] initWithFrame:CGRectMake(0, 0, 0, 160)];
    ...
    _pathCircle = [[CircleView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    ...
    _fillCircle = [[CircleView alloc] initWithFrame:_pathCircle.frame];
...
- (void)customInit
{
    ...
    // TextView (area where you write text)
    ...
    _colorPanel.backgroundColor = [UIColor colorWithRed:255.0/255.0 green:72.0/255.0 blue:72.0/255.0 alpha:0.5];
    _colorPanel.opaque = true;
    [_scrollView addSubview:_colorPanel];
    [self setColorPanel];
    
    
    _fontPickerView = [[FontPickerView alloc] initWithFrame:CGRectMake(self.width * 2, 0, self.width, self.height)];
    _fontPickerView.delegate = self;
    _fontPickerView.sizeComponentHidden = true;
...
- (void)setSelectedMode:(UIView *)selectedMode
{
    if(selectedMode != _selectedMode){
        ...
        _selectedMode.backgroundColor = [[ImageEditorTheme theme] toolbarSelectedButtonColor];
...
- (void)keyBoardWillChange:(NSNotification *)notificatioin withTextViewHeight:(CGFloat)height
{
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState | (animationCurve<<16)
        animations:^{
...
- (void)textViewDidChange:(UITextView*)textView
{
    ...
    if(selection.location+selection.length == textView.text.length && [textView.text characterAtIndex:textView.text.length-1] == '\n') {
        ...
        [textView scrollRectToVisible:CGRectMake(0, textView.contentSize.height - 1, 1, 1) animated:true];
    } else {
