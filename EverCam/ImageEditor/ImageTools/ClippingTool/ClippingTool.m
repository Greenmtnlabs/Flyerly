...
static NSString* const kClippingToolRatios = @"ratios";
static NSString* const kClippingToolSwapButtonHidden = @"swapButtonHidden";
...
@interface CLRatioMenuItem : ToolbarMenuItem
...
{
    return NSLocalizedStringWithDefaultValue(@"ClippingTool_DefaultTitle", nil, [ImageEditorTheme bundle], @"Crop", @"");
}
...
{
    return @[
             @{kClippingToolRatioValue1:@0, kClippingToolRatioValue2:@0, kClippingToolRatioTitleFormat:NSLocalizedStringWithDefaultValue(@"ClippingTool_ItemMenuCustom", nil, [ImageEditorTheme bundle], @"Custom", @"")},
...
{
    return @{kClippingToolRatios:[self defaultPresetRatios], kClippingToolSwapButtonHidden:[self defaultSwapButtonHidden]};
}
...
- (void)setup {
    ...
    BOOL swapBtnHidden = [self.toolInfo.optionalInfo[kClippingToolSwapButtonHidden] boolValue];
    ...
    if(!swapBtnHidden){
        ...
        UIView *btnPanel = [[UIView alloc] initWithFrame: CGRectMake(_menuScroll.right, 0, 70, _menuScroll.height)];
        btnPanel.backgroundColor = MAIN_COLOR;
        btnPanel.clipsToBounds = true;
        
        [_menuContainer addSubview:btnPanel];
        
        
        UIButton *rotateButt = [UIButton buttonWithType:UIButtonTypeCustom];
        rotateButt.frame = CGRectMake(0, 0, 35, 35);
        rotateButt.center = CGPointMake(btnPanel.width/2, btnPanel.height/2);
        [rotateButt addTarget:self action:@selector(pushedRotateBtn:) forControlEvents:UIControlEventTouchUpInside];
        [rotateButt setBackgroundImage:[UIImage imageNamed:@"clipping_rotate"] forState:UIControlStateNormal];
        rotateButt.adjustsImageWhenHighlighted = true;
        [btnPanel addSubview: rotateButt];
    }
    
    
    // Initialize the Grid =============
    _gridView = [[ClippingPanel alloc] initWithSuperview:self.editor.imageView.superview frame:self.editor.imageView.frame];
...
- (void)setCropMenu
{
    ...
    NSArray *ratios = self.toolInfo.optionalInfo[kClippingToolRatios];
    BOOL swapBtnHidden = [self.toolInfo.optionalInfo[kClippingToolSwapButtonHidden] boolValue];
...
- (void)setSelectedMenu:(CLRatioMenuItem *)selectedMenu
{
    if(selectedMenu != _selectedMenu){
        ...
        _selectedMenu.backgroundColor = [ImageEditorTheme toolbarSelectedButtonColor];
        
        _selectedMenu.layer.cornerRadius = 20;
...
{
    ClippingCircle *view = [[ClippingCircle alloc] initWithFrame:CGRectMake(0, 0, 75, 75)];
...
{
    if(animated){
        [UIView animateWithDuration:kImageToolFadeoutDuration animations:^{
        ...
        animation.duration = kImageToolFadeoutDuration;
    ...
    } else {   self.clippingRect = clippingRect;  }
}
...
- (void)refreshViews
{
    ...
    CGPoint center = iconImageView.center;
    //_iconView.center;
    CGFloat W, H;
    if(_ratio.ratio!=0){
        ...
        CGFloat maxW  = MAX(iconImageView.image.size.width, iconImageView.image.size.height);
        //MAX(_iconView.image.size.width, _iconView.image.size.height);
        W = 50 * iconImageView.image.size.width/ maxW;
        //_iconView.image.size.width / maxW;
        H = 50 * iconImageView.image.size.height / maxW;
        //_iconView.image.size.height / maxW;
    }
    //_iconView.frame
    iconImageView.frame = CGRectMake(center.x-W/2, center.y-H/2, W, H);
