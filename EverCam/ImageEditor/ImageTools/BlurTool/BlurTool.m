...
+ (NSString*)defaultTitle {
    return NSLocalizedString(@"BlurTool", @"");
}
...
- (void)setup {
   ...
   // [self.editor fixZoomScaleWithAnimated: true];
    ...
    _blurSlider.thumbTintColor = [UIColor blackColor];
    _blurSlider.minimumTrackTintColor = LIGHT_COLOR;
    _blurSlider.maximumTrackTintColor = LIGHT_COLOR;
...
- (void)executeWithCompletionBlock:(void(^)(UIImage *image, NSError *error, NSDictionary *userInfo))completionBlock {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIActivityIndicatorView *indicator = [ImageEditorTheme indicatorView];
...
- (void)setBlurMenu {
    ...
    NSArray *_menu = @[
            @{@"title":NSLocalizedString(@"BlurEffect_MenuItemNormal", @""),
            @"icon":[UIImage imageNamed:@"bl_normal"] },
                       
            @{@"title":NSLocalizedString(@"BlurEffect_MenuItemCircle", @""),
            @"icon":[UIImage imageNamed:@"bl_circle"] },
                       
            @{@"title":NSLocalizedString(@"BlurEffect_MenuItemBand", @""),
            @"icon":[UIImage imageNamed:@"bl_band"]  },
    ];
    
    NSInteger tag = 0;
    for(NSDictionary *obj in _menu){
        ToolbarMenuItem *view = [ImageEditorTheme menuItemWithFrame:CGRectMake(x, 0, W, H) target:self action:@selector(tappedBlurMenu:) toolInfo:nil];
        view.tag = tag++;
...
- (UIImage*)circleBlurImage:(UIImage*)image withBlurImage:(UIImage*)blurImage
{
    ...
    UIImage *mask = [UIImage imageNamed:@"circle"];
...
- (UIImage*)bandBlurImage:(UIImage*)image withBlurImage:(UIImage*)blurImage {
    UIImage *mask = [UIImage imageNamed:@"band"];
...
#pragma mark - BLUR CIRCLE IMPLEMENTATION ===============================
...
- (void)drawRect:(CGRect)rect
{
    ...
    self.alpha = 1;
    [UIView animateWithDuration:kImageToolFadeoutDuration  delay:1 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction animations:^{
        self.alpha = 0;
    } completion:^(BOOL finished) {  }];
...
#pragma mark - BLUR BAND IMPLEMENTATION ===============================
