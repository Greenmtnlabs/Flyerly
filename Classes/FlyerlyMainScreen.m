...
@synthesize btnInvite, btnSaved, btnShared, btnSocial,vwSearch;
...
-(void)loadAdsTiles{
    ...
    for(int j=0;j<addsCount; j++){
        ...
        dispatch_async( dispatch_get_main_queue(), ^{
            ...
            //[self.bannerAdd[i] loadRequest:[GADRequest request]];
            //}
        });
    }
}
...
-(void)onShare:(id)sender
{
    ...
    if ( [[PFUser currentUser] sessionToken] )
    {
        ...
        if ([flyer isVideoFlyer])
        {
            ...
            shareArray = @[shareText, website,youtubeLink,shareImage];
        }
        else
        {
...
- (void) userPurchasesLoaded
{
    ...
    {
        if(inappviewcontroller != nil)
        {
             [self.navigationController presentViewController:inappviewcontroller animated:NO completion:nil];
        }
        ...
        if ([MFMessageComposeViewController canSendText])
           {
             ...
             NSString *message = @"I'm using the Flyerly app to create and share flyers on the go! Want to give it a try? \nhttp://greenmtnlabs.com/flyerly/index.html?i=FHgB9l98ab";
